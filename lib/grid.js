'use strict'

var creteBuffer   = require('gl-buffer')
var createShader  = require('gl-shader')
var shaders       = require('./shaders')

function Grid(plot, vbo, shader) {
  this.plot   = plot
  this.vbo    = vbo
  this.shader = shader
  this.ticks  = [[], []]
  this.lineWidth = [1, 1]
}

var proto = Grid.prototype

proto.draw = (function() {

  var DATA_SHIFT = [0,0]
  var DATA_SCALE = [0,0]
  var DATA_AXIS  = [0,0]

  return function() {
    var plot      = this.plot
    var gl        = plot.gl
    var vbo       = this.vbo
    var shader    = this.shader
    var ticks     = this.ticks
    var bounds    = plot._tickBounds
    var dataBox   = plot.dataBox
    var lineWidth = this.lineWidth

    for(var i=0; i<2; ++) {
      DATA_SHIFT[i] = 0.5 * (bounds[i]+bounds[i+2] - dataBox[i]+dataBox[i+2])
      DATA_SCALE[i] = 2.0 * (bounds[i+2] - bounds[i]) / (dataBox[i+2] - dataBox[i])
    }

    shader.bind()
    vbo.bind()
    shader.attributes.dataCoord.pointer()
    shader.uniforms.dataShift = DATA_SHIFT
    shader.uniforms.dataScale = DATA_SCALE

    var offset = 0
    for(var i=0; i<2; ++i) {
      DATA_AXIS[0] = DATA_AXIS[1] = 0
      DATA_AXIS[i] = 1
      shader.uniforms.dataAxis  = DATA_AXIS
      shader.uniforms.lineWidth = lineWidth[i]

      var size = ticks[i].length * 6
      gl.drawArrays(gl.TRIANGLES, offset, size)
      offset += size
    }
  }
})()


proto.update = (function() {
  var OFFSET_X = [1,  1, -1, -1,  1, -1]
  var OFFSET_Y = [1, -1,  1,  1, -1, -1]

  return function(options) {
    var ticks  = options.ticks
    var bounds = options.bounds
    var data   = new Float32Array(6 * 3 * (ticks[0].length + ticks[1].length))

    var ptr    = 0
    var gridTicks = [[], []]
    for(var dim=0; dim<2; ++dim) {
      var localTicks = gridTicks[dim]
      var axisTicks = ticks[dim]
      var lo = bounds[dim]
      var hi = bounds[dim+2]
      for(var i=0; i<axisTicks.length; ++i) {
        var x = (axisTicks[i].t - lo) / (hi - lo)
        localTicks.push(x)
        for(var j=0; j<6; ++j) {
          data[ptr++] = x
          data[ptr++] = OFFSET_X[j]
          data[ptr++] = OFFSET_Y[j]
        }
      }
    }

    this.ticks = gridTicks
    this.vbo.update(data)
  }
})()

proto.dispose = function() {
  this.vbo.dispose()
  this.shader.dispose()
}

function createGrid(options) {
  var plot   = options.plot
  var gl     = plot.gl
  var vbo    = createBuffer(gl)
  var shader = createShader(gl, shaders.gridVert, shaders.gridFrag)
  var grid   = new Grid(plot, vbo, shader)
  return grid
}
