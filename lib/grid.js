...
proto.drawTickMarks = (function() {
  ...
  return function() {
    ...
    var yCount  = 6 * (yEnd - yStart)

    SCR_OFFSET[0]         = 2.0 * (viewBox[0] - tickMarkLength[0]) / screenWidth - 1.0
    SCR_OFFSET[1]         = 0
    TICK_SCALE[0]         = tickMarkLength[0] * pixelRatio / screenWidth
    TICK_SCALE[1]         = tickMarkWidth[0]  * pixelRatio / screenHeight
    ...
    SCR_OFFSET[0]         = 0
    SCR_OFFSET[1]         = 2.0 * (viewBox[1] - tickMarkLength[1]) / screenHeight - 1.0
    TICK_SCALE[0]         = tickMarkWidth[1]  * pixelRatio / screenWidth
    TICK_SCALE[1]         = tickMarkLength[1] * pixelRatio / screenHeight
    ...
    SCR_OFFSET[0]         = 2.0 * (viewBox[2] + tickMarkLength[2]) / screenWidth - 1.0
    SCR_OFFSET[1]         = 0
    TICK_SCALE[0]         = tickMarkLength[2] * pixelRatio / screenWidth
    TICK_SCALE[1]         = tickMarkWidth[2]  * pixelRatio / screenHeight
    ...
    SCR_OFFSET[0]         = 0
    SCR_OFFSET[1]         = 2.0 * (viewBox[3] + tickMarkLength[1]) / screenHeight - 1.0
    TICK_SCALE[0]         = tickMarkWidth[3]  * pixelRatio / screenWidth
    TICK_SCALE[1]         = tickMarkLength[3] * pixelRatio / screenHeight
  ...
  return function(options) {
    ...
    for(var dim=0; dim<2; ++dim) {
      ...
      for(var i=0; i<axisTicks.length; ++i) {
        if(zeroLineEnable[dim] && axisTicks[i].x === 0) {
          continue
        }
